...
const PREFS_KEY = 'labeltracker_pro_login_prefs';
...
const LoginPage: React.FC<LoginPageProps> = ({ onLogin }) => {
  ...
  useEffect(() => {
    ...
    if (storedPrefs) {
      try {
        ...
        if (prefs.rememberMe && prefs.username) {
          setUsername(prefs.username);
          setRememberMe(true);
        }
      } catch (e) {
        // Corrupted prefs, clear it
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (foundUser) {
      if (rememberMe) {
        localStorage.setItem(PREFS_KEY, JSON.stringify({ rememberMe: true, username: username }));
      } else {
        localStorage.removeItem(PREFS_KEY);
      }
      onLogin(foundUser);
        ...
        <div className="p-12">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex items-center">
              ...
              />
              <label htmlFor="remember-me" className="ml-3 block text-sm font-bold text-slate-500 cursor-pointer select-none">
                Remember Network ID
              </label>
            </div>
